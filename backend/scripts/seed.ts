...
const LOCALITIES = [
  {
    ...
    summary:
      "Dense office district with short commutes, strong food access, and higher weekday traffic.",
  ...
  },
  {
    ...
    summary:
      "Planned locality with newer housing, cafes, parks, and better value, but rain and traffic can affect commutes.",
  ...
  },
  {
    ...
    summary:
      "Residential, calmer than office hubs, with good food streets and moderate commute to Sector V.",
...
const NEGOTIATED_RENTS = [
  {
    property_id: null,
    locality_id: "loc-sector-v",
    listed_rent: 10000,
    negotiated_rent: 9200,
    broker_commission: 0,
    maintenance_charges: 0,
    hidden_costs: ["electricity extra"],
    created_at: NOW,
  },
  {
    property_id: null,
    locality_id: "loc-new-town",
    listed_rent: 15000,
    negotiated_rent: 13700,
    broker_commission: 7000,
    maintenance_charges: 1200,
    hidden_costs: ["move-in fee"],
    created_at: NOW,
  },
];
...
async function seed() {
  ...
  try {
    ...
    await db.collection("negotiated_rents").deleteMany({});

    await db.collection("localities").insertMany(LOCALITIES as any);
    await db.collection("properties").insertMany(PROPERTIES as any);
    await db.collection("negotiated_rents").insertMany(NEGOTIATED_RENTS as any);

    console.log("Seeded localities, properties, and negotiated rent examples successfully!");
  } finally {
    await client.close();
  }
}
