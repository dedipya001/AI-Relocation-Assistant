...
export function resolveProviderListingUrl(item: ProviderListingLike): string | null {
  ...
  if (source.includes("magicbricks")) {
    return `https://www.magicbricks.com/property-for-rent-in-${city}-pppfr`;
  }
  if (source.includes("nobroker")) {
    return `https://www.nobroker.in/property/rent/${city}/all`;
  }
  if (source.includes("housing")) {
    return `https://housing.com/rent/property-for-rent-in-${city}`;
  }
  if (source.includes("99acres") || source.includes("acres99")) {
    return `https://www.99acres.com/search/property/rent/${city}?res_com=R&preference=R`;
  }
  if (source.includes("telegram")) {
    return validHttpUrl(item.provider_url) ?? "https://t.me/";
  }

  return validHttpUrl(item.provider_url);
}

export function attachProviderUrls<T extends Record<string, any>>(item: T): T & {
  listing_url: string | null;
  source_url: string | null;
  provider_url: string | null;
} {
  ...
  return Object.assign(item, {
    listing_url: listing,
    source_url: direct ?? provider,
    provider_url: provider ?? listing,
  });
}
